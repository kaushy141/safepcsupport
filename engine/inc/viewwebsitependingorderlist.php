...
<?php }?>
  <div class="col-lg-12 mt-1">
    <div class="card">
      <div class="card-header"> <i class="fa fa-align-justify"></i> Pending Order List 
...
var data = {
	...
	var postdata = {
			action	:	"viewwebsitependingorderlist",
			filter  :   formdata
		}
...
table = $('#<?php echo $TABEL_ID;?>').DataTable( {
	...
	'fnCreatedRow': function (nRow, aData, iDataIndex) {
		//$(nRow).find('td').not(":last").attr('onclick', "Redirect('"+aData[10]+"')"); // or whatever you choose to set as the id
	},
	"rowCallback": function( row, data, index ) {$('td', row).css('background-color', data[9]);},
		"order": [[ 1, 'asc' ]],
		columnDefs: [
			{ targets: [4,5], orderable: false },
			{ targets: [ 3,4 ], className: "hidden-xs hidden-md hidden-sm visible-lg" }
		]
} );
