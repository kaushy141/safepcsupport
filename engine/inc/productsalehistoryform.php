<div class="row">
  <div class="col-sm-12">
    <div class="card">
      ...
      <form id="addbatchproductalerecordform" name="addbatchproductalerecordform" style="display:none;">
        <div class="card-block">
          ...
          <div class="row py-2 bg-primary">
			<div class="col-xs-12 col-sm-6 col-md-4">
              <div class="form-group">
                <label for="bpca_store_id">Purchase Store<sup>*</sup></label>
                <select id="bpca_store_id" name="bpca_store_id" class="form-control" size="1">
                  <?php
                $store = new Store(0);
			...
			<div class="col-xs-12 col-sm-6 col-md-4">
              <div class="form-group">				  
                <label for="bpca_sell_price">Selling Price<sup>*</sup></label>
				 <div class="input-group">
                <input class="form-control" id="bpca_sell_price" name="bpca_sell_price" maxlength="10" placeholder="Enter Selling Product price only" type="number" step="0.01" value=""><span class="input-group-addon">GBP</span></div>		  
              </div>
            </div>
            
			<div class="col-xs-12 col-sm-6 col-md-4">
              <div class="form-group">				  
                <label for="bpca_store_reference">Store Reference<sup>*</sup></label>
				 <div class="input-group">
                <input class="form-control" id="bpca_store_reference" name="bpca_store_reference" maxlength="30" placeholder="Enter Reference like order id" type="text" value=""><span class="input-group-addon">ORDER ID</span></div>		  
              </div>
            </div>
            
            <div class="col-xs-12 col-sm-6 col-md-4">
              <div class="form-group">
            <label for="bpca_sell_date">Selling Date/Time<sup>*</sup></label>
            <div class="input-group date">
              <input type='text' class="form-control" id="bpca_sell_date" name="bpca_sell_date" placeholder="YYYY-MM-DD" value="" />
              <span class="input-group-addon">
              <label style="margin-bottom:0px;" for="bpca_sell_date"><i class="fa fa-calendar fa-lg m-t-2"></i></label>
              </span> </div>
            <script type="text/javascript">
				$(function () {
					$('#bpca_sell_date').datetimepicker({
		...
		<div class="row bg-primary">
			<div class="col-sm-12">
             <div class="form-group">
                <label for="bpca_remark">Selling Remark<sup>*</sup></label>
                <textarea id="bpca_remark" name="bpca_remark" rows="4" class="form-control" placeholder="Enter selling remark like extra charge or shipping charges etc"></textarea>
              </div>
            </div>
          </div>
        ...
        <div class="col-xs-12 col-sm-6 col-md-4">
          <div class="form-group">
            <label for="bpca_is_returned">For product return only</label>
            <br/>
            <label for="bpca_is_returned"><i class="fa fa-history fa-lg m-t-2"></i> &nbsp;is this Product returned ?</label>
            <label class="switch switch-icon switch-pill switch-success pull-right">
              <input class="switch-input" id="bpca_is_returned" value="1" name="bpca_is_returned" type="checkbox">
              <span class="switch-label" data-on="Yes" data-off="No"></span> <span class="switch-handle"></span> </label>
          </div>
        </div>
        <div class="col-xs-12 col-sm-6 col-md-4" id="bpca_returned_date_box" style="display:none;">
          <div class="form-group">
            <label for="bpca_returned_date">Returned Date<sup>*</sup></label>
            <div class="input-group date">
              <input class="form-control" id="bpca_returned_date" name="bpca_returned_date" placeholder="YYYY-MM-DD" value="" type="text">
              <span class="input-group-addon">
              <label style="margin-bottom:0px;" for="bpca_returned_date"><i class="fa fa-calendar fa-lg m-t-2"></i></label>
              </span> </div>
            <script type="text/javascript">
                    $('#bpca_returned_date').datepicker({
                        format: "yyyy-mm-dd",
						autoclose:true,
        ...
        <div class="card-footer">
		  ...
		  <?php if(isset($bpca_product_id) && $bpca_product_id!=0):?> 
			<a href="#"  data-toggle="modal" data-target="#appModal"  class="btn btn-primary btn-sm" onClick="openChatLogForm('<?=$bpca_product_id?>|B', '<?=$product_type?> Log Report')">Comments</a>
		<?php endif;?>
        </div>
        <input type="hidden" id="action" name="action" value="<?=isset($action)?$action:"savebatchproductrecord";?>"  />
        <input type="hidden" id="formcode" name="formcode" value="<?=App::getFormcode();?>"  />
        <input type="hidden" id="bpca_product_id" name="bpca_product_id" value="<?=isset($bpca_product_id)?$bpca_product_id:"0";?>"  />
		<input type="hidden" id="bpca_id" name="bpca_id" value="0"  />
        <input type="hidden" id="bpca_customer_id" name="bpca_customer_id" value=""  />
        <input type="hidden" id="bpca_customer_address_id" name="bpca_customer_address_id" value=""  />
      </form>
    </div>
  </div>
  <!--/col--> 
...
<script type="text/javascript">
$("#bpca_is_returned").on("change", function(){
	if($(this).is(":checked")){
		$("#bpca_returned_date_box").show();
	}else{
		$("#bpca_returned_date_box").hide();
	}			
});  
       ...
       <div class="block-fluid table-sorting clearfix">
        <table id="tbldatatable" class="table table-striped">
          ...
          <tbody id="sellingrecords">
          	<?php 
			$bpsh = new BatchProductSaleHistory(0);
			echo $bpsh->getRecords($bpca_product_id);
			?>
          </tbody>
        </table>
...
{
	var formFields	=	"customer_email, customer_phone, customer_type_id, customer_fname, customer_lname, customer_address_postcode, customer_address_street_number, customer_address_route, customer_address_administrative_area, customer_address_country, bpca_store_id, bpca_sell_price, bpca_sell_date";
	
	if(validateFields(formFields))
	{
		var data={
			action	:	$("#action").val(),
			bpca_is_returned : $("#bpca_is_returned:checked").val()
		};
		
		data = $.extend(data, $("#addbatchproductalerecordform").serializeFormJSON());
...
function viewSellDetail(bpca_id){	
	var data={
		action	:	'product/getbatchproducthistorydetail'	,
		bpca_id :	bpca_id
	};
...
function removeProductSalesHistory(bpca_id){
	var data={
		action	:	'product/removeproductsalehistory'	,
		bpca_id :	bpca_id
	};
...
function openHistoryForm(arr){
	...
	$("#addbatchproductalerecordform").slideDown();
}
