...
class Customer extends DB
{
 ...
 WHERE a.`customer_id` = '" . $this->id . "' GROUP BY a.`customer_id`";
	...
	function getCustomerCountryFilter()
	{
		$sql = "SELECT COUNT(a.`customer_id`) as record,  b.`customer_address_country` FROM `" . $this->table_name . "` AS a INNER JOIN `app_customer_address` AS b ON a.`customer_id` = b.`customer_id` GROUP BY b.`customer_address_country` ORDER BY b.`customer_address_country`";
		$dbc 	= 	new DB();
		$result	=	$dbc->db_query($sql);
		$record_array = array();
		if ($dbc->db_num_rows() > 0) {
		...
		$this->aColumn = array(
			"TABLES" => array(
				...
				"`app_customer_address`" => array(
					"column" => array("`customer_address_street_number`", "`customer_address_route`", "`customer_address_locality`", "`customer_address_administrative_area`", "`customer_address_country`", "`customer_address_postcode`", "`customer_address_geo_location`"),
					"reference" => "c",
					"join" => array("type" => "INNER JOIN", "table" => "`app_customer`", "on" => array("`customer_id`" => "`customer_id`"))
				)
			),
