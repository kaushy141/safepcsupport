...
function cst()
{
	global $data, $app;
	$cst = new Customer();
	if($customer_id = $cst->getIdByEmail(md5($data['u'])))
	{
		$customer = new Customer($customer_id);
		$info = $customer->getDetails();
		if($info['user_is_email_verified'] == 1)
		{
			if(md5($info['customer_email'].$info['customer_id'])==$data["l"] && md5($info['customer_id'].$info['customer_email'])==$data["i"])
			{
				Activity::add("verified email id",NULL, 0, $info['customer_id'], 'C'); 
				$customer->update(array("customer_is_email_verified" => "1"));
				return "success::Hi, $info[customer_fname], Your Email Activated Successfully ..!!!<br/><h4>Please login here to continue... <a href='".$app->basePath("customer-login.php")."'>Login</a></h4>";
			}
			else
				return "error::Activation couldn't Completed ..!!!<br/><h4>Activation Link Expired or You have Completed Logged in Activity</h4>";
		}
		else
			return "error::Email already verified<br/><h4>Please login here to continue... <a href='".$app->basePath("customer-login.php")."'>Login</a></h4>";
	}
	else
	{
