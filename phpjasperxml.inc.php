...
class PHPJasperXML {
	...
	private $report_count=1;
	public $protection = null;		//### New declaration By Kaushal (variable exists in original too)
	public $certificate = null; 	//### New declaration By Kaushal (variable exists in original too)
	public $certificateInfo	= null; //### New declaration By Kaushal (variable exists in original too)
	public $certificateImage	= null; //### New declaration By Kaushal (variable exists in original too)
	public $certificatePrivateKey = null;
	private $group_count = array();
	 //### New declaration
...
//### End of modification
    ...
    public function outpage($out_method="I",$filename="") {
        ...
        if($this->lang=="cn") {
			...
			if($this->protection)
				$this->pdf->SetProtection(array('print','modify'), $this->protection);			
        }
        else {
...
//variable not yet implemented
    ...
    public function background() {
		if(is_array($this->arraybackground) && count($this->arraybackground))
    ...
    public function pageHeader($headerY='') {
        ...
        $this->pdf->AddPage();
		if($this->certificate){
			$this->pdf->setSignature($this->certificate, $this->certificatePrivateKey, '', '', 2, $this->certificateInfo);
			//$this->pdf->Image($this->certificateImage, 540, 60, 15, 15, 'PNG');
			$this->pdf->setSignatureAppearance(400, 80, 160, 80);
			$this->pdf->addEmptySignatureAppearance(400, 80, 160, 80);
		}
        $this->background();
...
//                if($this->SubReportCheckPoint>0)
	...
	//					  $checkpoint=$this->maxpagey['page_'.($this->pdf->getNumPages()-1)]+$gfoot;						      
			...
			echo"<script type='text/javascript'>alert('Records not found...!!!!');window.close();</script>";
            exit(0);
        }
...
//		$this->pdf->Cell(300,10,"$currentY+$gfootheight > $pageheight-$bottommargin-$footerheight",1);
      ...
      //                   $gfoot= $this->showGroupFooter($this->maxpagey['page_'.($this->pdf->getPage()-1)]);
		//				  $checkpoint=$this->maxpagey['page_'.($this->pdf->getNumPages()-1)]+$gfoot;						      
			...
			//$_SESSION['nodata']="No data found.";
			//header("location:generate_report_pwsd_changed.php");
            //echo "No data found : On line number 2952.";
			//echo"<a href='generate_report_pwsd_changed.php'>No data, Go Back</a>";
			echo"<script type='text/javascript'>alert('Records not found...!!!');window.close();</script>";
            exit(0);
        }
